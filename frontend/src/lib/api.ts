...
export const dashboardApi = {
  getSummary: (yearMonth?: number) =>
    fetchApi<DashboardSummary>(`/dashboard/summary${yearMonth ? `?yearMonth=${yearMonth}` : ''}`),
  
  getNetWorthHistory: () =>
    fetchApi<NetWorthSnapshot[]>('/dashboard/net-worth-history'),
  
  recordNetWorthSnapshot: () =>
    fetchApi<NetWorthSnapshot>('/dashboard/record-snapshot', { method: 'POST' }),
}
...
export const exportApi = {
  ...
  exportExpenses: (yearMonth?: number, format: 'csv' | 'json' = 'csv') => {
    ...
    return fetch(`${API_BASE}/export/expenses?${params}`, { credentials: 'include' })
      .then(res => res.blob())
  },
}
...
export const stocksApi = {
  getQuote: (ticker: string) =>
    fetchApi<StockQuote>(`/stocks/quote/${ticker}`),
