  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="card-glow">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Debt-to-Asset</CardTitle>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          >
            <Camera className={`h-4 w-4 mr-2 ${recordSnapshotMutation.isPending ? 'animate-pulse' : ''}`} />
            Record Snapshot
          </Button>
        </CardHeader>
        <CardContent>
                      ...
                      return (
                        <div className="bg-card border border-border rounded-lg p-3 shadow-lg">
                          ...
                          <p className="text-emerald-500 text-sm">
                            Assets: {formatCurrency(payload[0]?.payload?.assets || 0)}
                          </p>
                          <p className="text-rose-500 text-sm">
                            Liabilities: {formatCurrency(payload[0]?.payload?.liabilities || 0)}
                          </p>
                          <Separator className="my-2" />
                          <p className="font-bold">
                            Net Worth: {formatCurrency(payload[0]?.value as number || 0)}
                          </p>
                        </div>
                      )
                    }
      ...
      {stockAssets.length > 0 && (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            >
              <RefreshCw className={`h-4 w-4 mr-2 ${isRefreshingAssets ? 'animate-spin' : ''}`} />
              Refresh Prices
            </Button>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    <th className="text-left py-3 px-2 font-medium text-muted-foreground">Ticker</th>
                    ...
                    <th className="text-right py-3 px-2 font-medium text-muted-foreground">Ownership</th>
                  </tr>
                </thead>
                <tbody>
                  {stockAssets.map((asset) => {
                    ...
                    return (
                      <tr key={asset.id} className="border-b last:border-0">
                        <td className="py-3 px-2 font-mono font-medium">{asset.ticker}</td>
                        <td className="py-3 px-2">{asset.name}</td>
                        <td className="py-3 px-2 text-right tabular-nums">{asset.quantity}</td>
