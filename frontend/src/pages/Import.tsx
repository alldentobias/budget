...
export function ImportPage() {
  ...
  const handleUpload = () => {
    ...
    uploadMutation.mutate({ file: uploadedFile, extractor: selectedExtractor, yearMonth: targetYearMonth })
  }
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button 
                ...
                disabled={!uploadedFile || !selectedExtractor || uploadMutation.isPending}
                className="w-full"
              >
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between flex-wrap gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button onClick={() => commitMutation.mutate()} disabled={commitMutation.isPending || nonDuplicateStaged.length === 0}>
                    {commitMutation.isPending ? 'Committing...' : `Commit ${nonDuplicateStaged.length} Expenses`}
                  </Button>
                </div>
              </div>
            </CardContent>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b bg-muted/50">
                        <th className="p-3 w-10">
                          <Checkbox
                            checked={selectedStaged.size === nonDuplicateStaged.length}
                    ...
                    <tbody>
                      {nonDuplicateStaged.map((staged) => (
                        <tr key={staged.id} className="border-b last:border-0 hover:bg-muted/30">
                          <td className="p-3">
                            <Checkbox
          ...
          {duplicateStaged.length > 0 && (
            <Card className="border-amber-500/50">
              <CardHeader>
                ...
                <CardDescription>
                  These transactions match existing expenses. They will not be imported.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-0">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {duplicateStaged.map((staged) => (
                        <tr key={staged.id} className="border-b last:border-0 opacity-60">
